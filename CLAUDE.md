...
- `npm run register_commands` — register Discord slash commands (requires `.env` with `WIKIPEDIAN_CLIENT_ID` and `WIKIPEDIAN_TOKEN`)
...
Command registration is handled separately in `register_commands.js` (plain JavaScript, not TypeScript).
